...
| **Financial Markets / Applied Finance** | Coursera: *Python for Finance & Investment Fundamentals* (EDHEC Business School) | *Python for Finance* – **Yves Hilpisch** | Apply mathematics and Python to real financial data | Backtesting, time series, arbitrage, pricing | `option_surfaces_dashboard.ipynb` → Volatility surfaces and Greeks visualization |
| **Market & Macro Fundamentals** | *Bloomberg Market Concepts (BMC)* | — *(Bloomberg Learning Portal)* | Understand bonds, equities, macro indicators | Integrate real market and Bloomberg data | — *(LinkedIn badge / CV highlight)* |
| **Algorithmic Trading / Professional Application** | *EPAT – Executive Programme in Algorithmic Trading* (QuantInsti) | — *(Internal modules + Shreve + Hilpisch)* | Develop, test, and deploy automated trading systems | Strategy execution, backtesting, automation | `ub_smart_agent_system.ipynb` → Multi-agent trading system (UB futures, order flow, news) |
